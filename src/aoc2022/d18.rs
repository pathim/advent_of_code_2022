...
pub fn f(file:std::fs::File)->crate::AocResult{
    ...
    let cubes:std::collections::HashSet<Cube>=input.lines().map(|x| x.unwrap().parse().unwrap()).collect();
    let res1:usize=cubes.iter().map(|c| c.neighbours().iter().filter(|n| !cubes.contains(n)).count()).sum();

    let min=cubes.iter().fold((i32::MAX,i32::MAX,i32::MAX), |r,c| (r.0.min(c.0),r.1.min(c.1),r.2.min(c.2)));
    let max=cubes.iter().fold((i32::MIN,i32::MIN,i32::MIN), |r,c| (r.0.max(c.0),r.1.max(c.1),r.2.max(c.2)));
    ...
    let res2:usize=cubes.iter().map(|c| c.neighbours().iter().filter(|n| is_outside(n, &outside, min, max)).count()).sum();
